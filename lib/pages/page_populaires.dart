...
class PagePopulaires extends StatefulWidget {
  const PagePopulaires({super.key});

  @override
  State<PagePopulaires> createState() => _PagePopulairesState();
}

class _PagePopulairesState extends State<PagePopulaires> {
  List<Movie> _films = [];
  ...
  Future<void> _loadMovies() async {
    ...
    try {
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = 'Erreur lors du chargement des films populaires: $e';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const IndicateurChargement(
        message: 'Chargement des films populaires...',
      );
    }
    ...
    if (_films.isEmpty) {
      return const MessageErreur(message: 'Aucun film populaire trouvé');
    }
